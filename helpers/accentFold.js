...
module.exports.accent_fold = (s) => {
	if (!s) { return ''; }
    let ret = '';
    for (let i=0; i<s.length; i++) {
		ret += accent_map[s.toLowerCase().charAt(i)] || s.toLowerCase().charAt(i);
    }
    return ret;
};
...
//
module.exports.accent_folded_hilite = (str, q) => {
  let str_folded = accent_fold(str).toLowerCase().replace(/[<>]+/g, '');
  let q_folded = accent_fold(q).toLowerCase().replace(/[<>]+/g, '');
  ...
  for (let i=0; i<hilite_hints.length; i++) {
